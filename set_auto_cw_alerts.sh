...
if [ -z “$AutoScalingGroupName” ];
then
aws cloudwatch — region $region put-metric-alarm — alarm-name ${AlarmPrefix}_mem_used_percent_$InstanceID — alarm-description “Alert on Memory usage 80” — metric-name mem_used_percent — namespace CWAgent — statistic Average — period 300 — threshold 80 — comparison-operator GreaterThanThreshold — dimensions “Name=InstanceId,Value=$InstanceID” “Name=ImageId,Value=$AMIID” “Name=InstanceType,Value=$InstanceType” — evaluation-periods 1 — alarm-actions arn:aws:sns:$region:<AWSACCOUNTID>:CloudwatchMonitoring — unit Percent
else
aws cloudwatch — region $region put-metric-alarm — alarm-name ${AlarmPrefix}_mem_used_percent_$InstanceID — alarm-description “Alert on Memory usage 80” — metric-name mem_used_percent — namespace CWAgent — statistic Average — period 300 — threshold 80 — comparison-operator GreaterThanThreshold — dimensions “Name=InstanceId,Value=$InstanceID” “Name=AutoScalingGroupName,Value=$AutoScalingGroupName” “Name=ImageId,Value=$AMIID” “Name=InstanceType,Value=$InstanceType” — evaluation-periods 1 — alarm-actions arn:aws:sns:$region:<AWSACCOUNTID>:CloudwatchMonitoring — unit Percent
fi
#END Of Script
